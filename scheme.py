...
def make_let_frame(bindings, env):
    ...
    # BEGIN Question 16
    formals_pair, val_pair = nil, nil
    while bindings:
        bind = bindings.first
        check_form(bind, 2, 2)
        formals_pair = Pair(bind.first, formals_pair)
        val_pair = Pair(scheme_eval(bind.second.first, env), val_pair)
        bindings = bindings.second
    check_formals(formals_pair)
    return env.make_child_frame(formals_pair, val_pair)
...
def check_formals(formals):
    ...
    while formals != nil:
        first = formals.first
        if first not in lst and scheme_symbolp(first):
            lst.append(first)
